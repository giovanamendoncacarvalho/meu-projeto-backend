...
//É utilizado para enviar dados ou realizar ações que alteram o estado do servidor,
//como neste caso, onde o jogador realiza uma ação (como acionar um comportamento de ataque)
//que é o método attack() da classe Player.
//A função de callback recebe dois parâmetros: req (requisitação) e res (resposta)

app.post("/player/attack", (req: Request, res: Response) => {
const attackMessage = player1.attack(); //Chama o método de attack() do jogador
//Retorna uma resposta JSON com a mensagem do ataque para o cliente que fez a requisição
res.json({
...
//Quando o usuário acessar a rota "/player/damage", o servidor chamará o método takeDamage() do jogador
...
const damageMessage = player1.takeDamage(damage); //Chama o método takeDamage() do jogador
//Retorna uma resposta JSON com a mensagem do dano recebido para o cliente que fez a requisição
res.json({
